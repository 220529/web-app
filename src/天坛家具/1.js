  ...
  templateListData.value
    ...
    .forEach((v: any) => {
      if (v.fitUse === 4) {
        ...
        if (
          !productId?.length ||
          productId.includes(orderData.value.productId)
        ) {
...
const changeTemplateListData = computed(() => {
  return templateListData.value
    .filter((v: any) => v.status === 1 && [6].includes(v.fitUse))
    .map((v) => {
      return getCardData(v);
    });
});
...
const cardListData = computed(() => {
  return contractListData.value.map((v: any) => {
    ...
    if (
      ...
      if (
        curRole.value.type === "finance" ||
        functions.value.includes("order:detail:contract:download")
      ) {
  ...
  return {
    ...
    btns: [
      (useTemplateIds.value.includes(v.templateId) ||
        useTemplateTypes.value.includes(v.houseType)) &&
      v.houseState2 !== 1 &&
      v.fitUse !== 6
        ? {
            text: "当前模板类型已使用",
...
function openDeleteConfirm(contractFile: any) {
  ...
  confirmContent.value = `确认删除文件-${
    contractFile.ttContractTitle || "未获取到名称"
  }-吗？`;
  showConfirm.value = true;
}
...
function handleTipConfirm() {
  if (tipType.value === "inputRevokeReason") {
    ...
    confirmContent.value = `确认撤销签署-${
      curData.value.ttContractTitle || "未获取到名称"
    }-吗？`;
    showConfirm.value = true;
    showTip.value = false;
...
const materialContractTemplateFormDialogRef =
  ref<InstanceType<typeof MaterialContractTemplateFormDialog>>();
const changeContractTemplateFormDialogRef =
  ref<InstanceType<typeof ChangeContractTemplateFormDialog>>();
...
async function handleChooseTemplateType(chooseConfig: any) {
  ...
  if (
    !isType3 &&
    !isType4 &&
    chooseTemplates.filter((v: any) => !v.signFlow).length > 0
  ) {
  ...
  if (
    !isType3 &&
    !isType4 &&
    chooseTemplates.filter((v: any) => v.signFlow?.signFlowStatus === 1)
      .length > 0
  ) {
...
function getPdfName() {
  return `${chooseTemplateConfig.value.name}-${
...
async function handleChooseFile(file: any) {
  ...
  const url: string = (await getCoverImage(
  ...
  if (file.type === 2) {
    ...
    confirmContent.value = `确认选择-${
      files.value.find((v: any) => v.id === file.id)?.name || "未获取到名称"
    }-创建施工图纸模板文件吗？`;
    showConfirm.value = true;
  }
}
...
async function handleChoosePrintRecord() {
  ...
  // 原有的逻辑：直接合成明细PDF
  await handleChoosePrintRecordDirect();
}
...
async function handleChoosePrintRecordWithContract() {
  ...
  try {
    ...
    // 步骤4：上传最终文件
    const contractFile = new File([finalPdf], `${getPdfName()}.pdf`, {
      type: "application/pdf",
    });

    await uploadContract(
      contractFile,
      chooseCoverConfig.value.templateId,
      contractFileId,
      payload
    );
    ...
    (window as any).$wujie?.bus.$emit(
      "zhqUploadFile",
      {
        ...
        otherData: {
          ...
          targetId: orderData.value.id,
          ttContractNo: orderData.value.number,
          ttContractTitle: contractFile.name.slice(
            0,
            contractFile.name.lastIndexOf(".")
          ),
          useTemplate: 1,
          contractType: 6,
          docTemplateId,
          ...
          changeIds: chooseRecords.value.map((r: any) => r.id).join(",") || "",
        },
      },
      (res: any) => {
...
// 原有的直接合成逻辑
async function handleChoosePrintRecordDirect() {
...
async function getPdfUnitArray(
  url: string,
  tokenInfo: any
...
<template>
  <DrawerLayout v-model="modelValue" :width="1180">
    <v-card>
      <v-toolbar dark class="text-#333 relative z-2" color="#fff">
        ...
        <v-toolbar-items>
          ...
          <v-btn
            ...
            v-permission:orderInfo.id="{
              COMPLETION_PROCESS_PERMISSIONS: orderData?.id,
            }"
          >
      ...
      <div
        class="wfull bg-#f4f4f4 hc-100vh-64px overflow-auto beauty-scroll relative z-1"
      >
        <Empty v-if="contractListData.length === 0" text="暂未发起合同" />
        <Empty v-if="templateListData.length === 0" text="暂无合同模板" />
        <div
          ref="cardListRef"
          class="cardList flex flex-wrap gap-20px pa20px"
          v-else
        >
          <CustomCard
            v-for="(item, index) in cardListData"
            :key="index"
            :list-width="cardListWidth"
            :min-width="280"
            :data="item"
          />
          <!-- 文件选择 -->
    ...
    <DialogLayout
      ...
      header-icon="i-mdi-contract-sign"
      :height="700"
      :width="1200"
      bg="#f4f4f4"
      hide-footer
      no-padding
      title="选择模板(同类型只能签署一个)"
    >
      <div
        class="h50px wfull bg-#fafafa px-10px shadow-sm flex items-center r-z-2"
      >
        <v-chip-group
          v-model="chooseTemplateType"
        ...
        >
          <v-chip
            ...
            v-for="(item, index) in allTemplateTypes"
            :key="index"
          >
            {{ item.name }}
          </v-chip>
      ...
      <div class="h574px overflow-auto beauty-scroll r-z-1">
        <div
          class="cardList flex flex-wrap gap-20px pa20px"
          ref="templateCardListRef"
        >
          <CustomCard
            v-for="(item, index) in templateCardData"
            :key="index"
            :list-width="templateCardListWidth"
            :min-width="240"
            :data="item"
          />
        </div>
    ...
    <!-- 选择材料合同模版 -->
    <DialogLayout
      v-model="chooseMaterialTemplateDialogVisible"
      header-icon="i-mdi-contract-sign"
      :height="700"
      :width="1200"
      bg="#f4f4f4"
    ...
    >
      <Empty
        v-if="!materialTemplateListData.length"
        text="暂无启用的合同模板"
      />
      <!-- <div class="py10px">
        <div
      ...
      <div class="h574px overflow-auto beauty-scroll r-z-1">
        ...
        >
          <CustomCard
            v-for="(item, index) in materialTemplateListData"
            :key="index"
            :list-width="templateCardListWidth"
            :min-width="240"
            :data="item"
    ...
    <!-- 选择变更明细合同模版 -->
    <DialogLayout
      v-model="chooseChangeMaterDialogVisible"
      header-icon="i-mdi-contract-sign"
      :height="700"
      :width="1200"
      bg="#f4f4f4"
    ...
    >
      ...
      <div class="h574px overflow-auto beauty-scroll r-z-1">
        ...
        >
          <CustomCard
            v-for="(item, index) in changeTemplateListData"
            :key="index"
            :list-width="templateCardListWidth"
            :min-width="240"
            :data="item"
    ...
    <DialogLayout
      v-model="showTip"
      header-icon="mdi-alert-circle-outline"
      :title="tipDialogTitle"
      :width="dialogWidth"
      :height="dialogHeight"
      :hide-footer="['showReason'].includes(tipType)"
    ...
    >
      ...
      <div
        v-if="['inputRescissionReason', 'inputRevokeReason'].includes(tipType)"
      >
        <v-select
          v-if="tipType === 'inputRescissionReason'"
          class="mt10px"
          hide-details
          v-model="chooseType"
          placeholder="请选择解约原因"
          :items="rescissionReasonList"
          item-title="name"
          item-value="id"
          label="请选择解约原因"
          required
    ...
    >
      <Empty
        v-if="customTemplateList.length === 0"
        text="暂未配置可选择的模板类型"
      />
      <div class="chooseCardList flex flex-wrap gap-20px pa-20px flex-start">
        <div
          v-for="(c, i) in customTemplateList"
          :key="i"
          @click="handleChooseTemplateType(c)"
        ...
        >
          ...
          <div
            class="chooseBtn px-16px py-4px b-1 rounded-100px text-14px text-#3175fb"
          >
            选择
          </div>
        </div>
    ...
    <DialogLayout
      v-model="showChooseFiles"
      :width="710"
      :height="600"
      header-icon="mdi-file-image"
      title="选择图纸图片组或图纸PDF"
      :hide-header="true"
      :hide-footer="true"
    >
      <div class="fileList flex gap-10px flex-wrap">
        ...
        <div
          class="fileItem pa5px cursor-pointer bg-#f4f4f4 w120px rounded-8px hover:bg-blue-100"
        ...
        >
          <div class="icon flex-center text-60px">
            ...
            <div
              ...
              class="i-mdi-file-image text-blue"
            ></div>
          </div>
          <div
            class="text-center text-13px line-clamp-2 h40px items-center flex flex-center"
          >
    ...
    <DialogLayout
      v-model="showChoosePrintRecord"
      :width="1200"
      :height="700"
      header-icon="i-mdi-list-box-outline"
      title="选择需要创建合同模板的打印记录"
      :btnValue="`确认选择完成(已选${chooseRecords.length})12`"
      no-padding
      float-footer
    ...
    >
      <div class="flex h550px">
        <div
          class="w200px overflow-auto beauty-scroll pa-10px relative z-2 shadow-md"
        >
          <div
            v-for="(b, i) in printTypeList"
            :key="i"
            @click="handleChooseShowRecordType(b)"
            class="flex mb-10px rounded-6px cursor-pointer pa-10px text-13px items-center hover:bg-#f4f4f4 transition"
            :class="
              chooseShowRecordType === b.type
          ...
          >
            <CustomImgIcon
              :icon="b.icon"
              :size="20"
              :color="chooseShowRecordType === b.type ? '#3175FB' : '#333'"
            />
            <div class="ml-6px">
        ...
        <div
          class="flex-1 relative z-1 text-#333 bg-#f4f4f4 overflow-auto beauty-scroll"
        >
          <Empty v-if="printRecordCardListData.length === 0" text="暂无记录" />
          <div class="cardList flex flex-wrap gap-15px pa15px">
            <CustomCard
              v-for="(item, index) in printRecordCardListData"
              :key="index"
              :list-width="740"
              :min-width="220"
              :data="item"
        ...
        <div class="w230px relative z-2 shadow-md">
          ...
          >
            <div class="cardList flex flex-wrap gap-15px justify-center">
              <Empty
                v-if="chooseRecordCardListData.length === 0"
                text="暂未选择"
              />
              <CustomCard
                v-for="(item, index) in chooseRecordCardListData"
                :key="index"
                :list-width="200"
                :min-width="190"
                :data="item"
    ...
    <Teleport to="html">
      <DialogLayout
        ...
        :height="800"
        header-icon="mdi-file-image"
        title="图纸图片组合并PDF_拖拽排序"
        btnValue="合并为图纸PDF模板文件"
        no-padding
        float-footer
        content-class="h100vh"
      ...
      >
        ...
        <div class="wfull h645px flex flex-wrap splitPaneContainer">
          <div
            id="split-0"
            :class="showImagesListType === 'big' ? ['w310px'] : ['wfull']"
          >
            <Draggable
              v-model="chooseImages"
              item-key="id"
              filter=".forbid"
              chosen-class="chosen"
              force-fallback="true"
              group="people"
              animation="300"
              class="overflow-auto beauty-scroll flex flex-wrap h650px gap-10px pa10px content-start"
              :move="onMove"
            >
              <template #item="{ element, index }">
                <div
                  class="flex-center flex-wrap cursor-pointer rounded-4px hover:bg-blue-100 overflow-hidden relative"
                  :class="[
                    {
                      forbid: element.id === 0,
                ...
                >
                  <div
                    class="wfull flex-center items-center"
                    :class="
                      {
                        small: 'h95px',
                        normal: 'h130px',
                  ...
                  <div
                    class="text-12px lh-16px absolute left-0px top-0px px-4px py-2px text-#fff bg-#000000 font-bold"
                  >
                    第{{ index + 1 }}页
                  </div>
                  <div
          ...
          <div
            id="split-1"
            ref="showImageContainerRef"
            class="showImage wc-100%-310px flex-center overflow-hidden hfull"
            v-if="showImagesListType === 'big'"
          >
            <img
              :src="checkImage?.url"
              class="wfull hfull object-contain"
              :style="{
    ...
    <MaterialContractTemplateFormDialog
      v-if="orderData?.id && orderData?.customerId && orderData?.customer"
      ...
      ref="materialContractTemplateFormDialogRef"
      @refresh="getUseTemplateList"
    />
    <ChangeContractTemplateFormDialog
      ref="changeContractTemplateFormDialogRef"
      v-if="orderData?.id && orderData?.customerId && orderData?.customer"
      :orderId="orderData.id"
      :customerId="orderData.customerId"
      :orderNumber="orderData.number"

 ...
 * @flowNodeId 1
 * @updateTime 2025-08-02 11:01:11
 * @requireModules d05tcbf4k0gt4rqz
 */
return (() => {
  let {
    rowData,
  ...
  if (action === '订单业绩') {
    return fetchOrderRecords({ orderId });
  }
  if (action === '人力业绩') {
    return fetchHumanRecords();
  }
  if (action === '纸质合同') {
    return fetchRelationContractFiles({ orderIds });
  }
  if (action === '电子合同1') {
    return fetchContractsByOrderId({ orderId, fitUses: [4, 5] });
  }
  if (action === '订单记录') {
    return fetchOrderRecordsByParams({
      where: {
        targetId: orderId,
        // type: 18,
        // status: 2,
  ...
  if (action === '月份记录') {
    return getChooseMonth();
  }
  async function getChooseMonth() {
    try {
      const fileName = '设计费汇总';
      const fileType = 'confirmShejifeiOrder';
      const date = new Date();
      // 24年4月xxxx
  ...
  async function fetchCompanyInfo() {
    const url = Export_d05tcbf4k0gt4rqz.getImage(26);
    return url;
  }
  async function fetchPrint() {
    try {
      // zebg3gst00btm1t0
      const res = await ctx.service.common.runFlowByParams({
        flowKey: 'zebg3gst00btm1t0',
        orderId: 3226,
      });
      return res;
    } catch (error) {
  ...
  async function createSuppliers() {
    try {
      ...
      const cpSuppliers = ctx.helper.copy(suppliers);
      // const res = await Promise.all();
      return suppliers;
      const dicts = await ctx.model.Dict.findAll({
        where: {
          type: 20,
          companyId: 11,
        },
      });
      const newDicts = ctx.helper.copy(dicts);
      return dicts;

      // return await Promise.all( newDicts.map( item => {
      //   return ctx.service.base.create( {
      //     model: "Dict",
      //     body: {
      //       "name": item.name,
      //       "companyConfig": item.companyConfig,
      //       "companyId": 26,
      //       "icon": item.icon,
      //       "sort": item.sort,
      //       "type": item.type,
...
LEFT JOIN departments as designGroup ON c.designGroupId = designGroup.id`,
    ...
    if (query.orderStatus) {
      ...
      // 已退单/作废状态 - 只有在orderStatus中明确包含时才查询
      if (orderStatusArray.includes('12')) completed.push(2);
      if (orderStatusArray.includes('13')) completed.push(3);

      // 其它正常状态
      if (orderStatusArray.includes('0')) status.push(0);
      if (orderStatusArray.includes('1')) status.push(1);
      if (orderStatusArray.includes('2')) status.push(2);

      // 特殊状态处理
      if (orderStatusArray.includes('11')) {
        // 工程状态为2的订单，但排除已退单/作废的
        specialConditions.push({
          [Op.and]: [
            { '$orderProjectStatus.targetId$': { [Op.ne]: null } },
            {
              [Op.and]: [
                { completed: { [Op.ne]: 2 } },
                { completed: { [Op.ne]: 3 } },
      ...
      // 如果有已退单/作废状态，添加对应的查询条件
      if (completed.length > 0) {
        conditions.push({
          completed: { [Op.in]: completed },
        });
      }

      // 如果有正常状态，添加对应的查询条件（排除已退单/作废的订单）
      if (status.length > 0) {
        conditions.push({
          [Op.and]: [
            { status: { [Op.in]: status } },
      ...
      if (conditions.length > 0) {
        if ($whereOrder[Op.and] && $whereOrder[Op.and].length > 0) {
          $whereOrder[Op.and].push({
            [Op.or]: conditions,
          });
        } else {
          $whereOrder[Op.or] = conditions;
        }
      }

      // 设置特殊状态的查询参数（用于后续的include处理）
      if (orderStatusArray.includes('11')) {
    ...
    if (query.productName) {
      const products = await ctx.model.Product.findAll({
        where: {
          name: { [Op.like]: `%${query.productName}%` },
        },
        attributes: ['id'],
      });
      const productIds = products.map((p) => p.id);
      if (productIds.length > 0) {
        $whereOrder.productId = { [Op.in]: productIds };
      } else {
        // 没有匹配的产品，直接返回空
        return { rows: [], count: 0 };
      }
    ...
    if (query.projectSendStatus) {
      if (Number(query.projectSendStatus) === 1) {
        $where.foremanId = {
          [Op.is]: null,
        };
    ...
    if (Object.keys(query).includes('isScheduling')) {
      // 如果orderStatus包含22，已在orderStatus逻辑处理，这里不再重复添加
      if (!(query.orderStatus && query.orderStatus.split(',').includes('22'))) {
        if (query.orderStatus) {
    ...
    if (Object.keys(query).includes('isStart')) {
      // 如果orderStatus包含21，已在orderStatus逻辑处理，这里不再重复添加
      if (!(query.orderStatus && query.orderStatus.split(',').includes('21'))) {
        if (query.orderStatus) {
    ...
    if (
      ...
      // 如果orderStatus包含11，需要设置required = false，避免INNER JOIN
      if (query.orderStatus && query.orderStatus.split(',').includes('11')) {
        includeItem.required = false;
      }

      // 如果orderStatus包含11，已在orderStatus逻辑处理，这里不再重复添加
      if (
        !(query.orderStatus && query.orderStatus.split(',').includes('11')) &&
        query.orderStatus &&
        query.projectStatus === 2
      ) {
        includeItem.required = false;
    ...
    if (
      ...
      if (
        ...
        if (
          [2, 3, 4, 5, 6, 7].includes(Number(queryWaitDistributionMaterNum)) ||
  ...
  async function fetchOrderRecordsByParams(params) {
    try {
      const records = await ctx.model.Record.findAll(params);
      ...
      return await ctx.model.Record.destroy({
        where: {
          id: {
  ...
  async function fetchOrderRecords({ orderId }) {
    try {
      ...
      return records.filter((record) => record.type === 31);
    } catch (error) {
      return {
        error: error.message,
      };
  ...
  // 获取订单纸质合同
  async function fetchRelationContractFile({ orderId }) {
    try {
      const record = await ctx.model.Record.findOne({
        where: { targetId: orderId, type: 31 },
      });
      let contract = null;
      if (record) {
        record.data = JSON.parse(record.data);
        const { relationContractDate, relationContractFile } = record.data;
        if (relationContractDate && relationContractFile) {
          contract = {
            date: relationContractDate,
            file: relationContractFile,
          };
        }
      }
      return contract;
  ...
  async function fetchRelationContractFiles({ orderIds }) {
    try {
      const res = await Promise.all(
        orderIds?.map(async (orderId) => {
          return fetchRelationContractFile({
            orderId,
          });
        }),
      );
  ...
  // 获取签署的合同
  async function fetchContractsByOrderId({ orderId, fitUses }) {
    try {
      const order = await ctx.model.Order.findByPk(orderId);
      const records = await ctx.model.Record.findAll({
        where: {
          type: 40,
          targetId: order?.companyId,
          fitUse: {
            [Op.in]: fitUses || [4, 5], // 4:主合同 5:质保协议
          },
        },
      });
      let contracts = await ctx.helper.parseApi({
        url: '/contracts',
        data: {
          sync: 1,
          page: 1,
      ...
      const templateIds = records?.map((v) => {
        v.data = JSON.parse(v.data);
        return v.data?.templateId;
      });
      // return {
      //   records,
      //   contracts,
      //   templateIds,
      // };
      contracts = contracts?.rows?.filter((v) => {
        return templateIds.includes(v.docTemplateId) && v.fileStatus === 2;
      });
      contracts =
        contracts?.filter((v) => {
          let { signFlowStatus, rescissionStatus } = v?.signFlow || {};
          return signFlowStatus === 2 && !rescissionStatus;
        }) || null;
      return contracts;
    } catch (error) {
      return {
  ...
  async function fetchContractsByOrderIds({
    orderIds,
    fitUses,
    multiple = false,
  }) {
    orderIds = uniqueArray(orderIds);
    try {
      const res = await Promise.all(
        orderIds?.map(async (orderId) => {
          return fetchContractsByOrderId({
            orderId,
            fitUses,
          });
        }),
      );
      if (orderIds.length === 1 && !multiple) {
        return res?.[0];
      }
      const result = {};
      orderIds.forEach((orderId, index) => {
  ...
  function uniqueArray(arr) {
    ...
    if (!Array.isArray(arr)) {
      console.warn('uniqueArray: 参数不是数组');
      return [];
    }
  ...
  async function fetchOrderDetail() {
    try {
      const order = await ctx.service.order.find(body.orderId);
      // const order = await ctx.model.Order.findByPk(orderId);
      return order;
    } catch (error) {
  ...
  async function fetchHumanRecords() {
    try {
      const records = await ctx.model.Record.findAll({
        where: { type, fitUse, houseType: houseType37 },
      });
      if (destroy !== 'Y') {
        return records;
      }

/**
 * @flowId 627
 * @flowName 获取人力_开工业绩提点数据
 * @flowNodeName 函数名称
 * @flowNodeType funEditNode
 * @flowNodeId 1
 * @updateTime 2025-02-07 09:15:55
 */
...
// groupType 2 设计
// departmentId 分店ID
const { groupType = 2, fitUse, departmentId } = body;
...
try {
  ...
  let [record, [rows], [smUsers], [dsmUsers], tichengRules] = await Promise.all([
  ...
  const groupsData = rows.reduce((acc, cur, i) => {
    ...
    monthSigningAmountObj[userId] += jsonData.amount.effectivePerformanceAmount;

    // 累计部门签单额度 新签部分
    departmentMonthTotalSigningAmountObj[groupId] =
      departmentMonthTotalSigningAmountObj[groupId] ?? 0;
    departmentMonthTotalSigningAmountObj[groupId] += jsonData.amount.effectivePerformanceAmount;
      ...
      ctx.helper
        ...
        .map((v, i) => {
          ...
          if (!isConfirm) {
            ...
            const { effectivePerformanceAmount = 0 } = v.amount;
            ...
            const designTichengAmount_tc_ffb = 50;
  ...
  function getSheets(res) {
    ...
    const sheetName = `${fitUseStr} 开工业绩确认 - 设计 ${isConfirm ? "（已录入）" : ""}`;
    return {
      name: sheetName,
      styles: {
        Header_Formatter: {

/**
 * @flowId 638
 * @flowName 获取人力_竣工业绩提点数据
 * @flowNodeName 函数名称
 * @flowNodeType funEditNode
 * @flowNodeId 1
 * @updateTime 2025-03-05 09:19:16
 */
...
try {
  ...
  const smSql = `
    ...
    WHERE EXISTS (
        SELECT 1
        FROM roles
        WHERE FIND_IN_SET(roles.id, users.roles) > 0
        AND roles.name LIKE '三级部门正职%'
    )
    AND FIND_IN_SET(${departmentId}, departmentId)
  `;
  const dsmSql = `
    SELECT *
    FROM users
  ...
  const resultSql = `
    SELECT
      r.*,
      DATE_FORMAT(eb.earliestBillCreatedAt, '%Y年%m月%d日 %H时%i分') as earliestBillCreatedAt
    FROM
      records AS r
    LEFT JOIN (
      SELECT targetId, MIN(createdAt) AS earliestBillCreatedAt, type, status
      FROM bills as eb
      WHERE
          eb.type in (302, 301)
        AND
          eb.status = 3
        GROUP BY targetId
      ) AS eb ON r.targetId = eb.targetId
    WHERE
          r.type = 54
      AND r.houseType = 5
      AND r.status IN ( 2, 6, 8 )
      ${isConfirm ? "AND (r.fitType = 2)" : "AND (r.fitType IS NULL)"}
      AND r.data LIKE '%designDepartmentId":${departmentId},"designGroupName%'
  `;

  let [[rows], [smUsers], [dsmUsers], tichengRules] = await Promise.all([
    ctx.model.query(resultSql),
    ctx.model.query(smSql),
    ctx.model.query(dsmSql),
  ...
  // 当前月份已录入，那么只展示 当前月份录入的
  if (isConfirm) {
    rows = rows.filter(v => {
      const data = JSON.parse(v.data || "{}") || {};
      const { designTichengAmount } = data;
      if (designTichengAmount?.fitUse === fitUse) {
        return true;
      }
      return false;
    });
  ...
  const customerIds = [...new Set(rows.map(v => JSON.parse(v.data)?.customer?.id).filter(Boolean))];

  const designGroupIds = [
    ...new Set(rows.map(v => JSON.parse(v.data)?.customer?.designGroupId).filter(Boolean)),
  ];
  ...
  const groupsData = rows.reduce((acc, cur, i) => {
    // id 不要
    const { id, ...jsonData } = JSON.parse(cur.data);
    ...
    jsonData.customer.storeManagerName = smUsers.map(v => v.name).join(",");
    jsonData.customer.deputyStoreManagerName = dsmUsers.map(v => v.name).join(",");
    ...
    // 生效业绩额: =竣工额-远程费-上楼费
    const realEffectivePerformanceAmount =
      amount.totalPrice - amount.upstairsFee - amount.remoteFee;
    ...
    const item = {
      ...
      realEffectivePerformanceAmount, // 竣工生效业绩额
    };
    ...
    monthSigningAmountObj[userId] += realEffectivePerformanceAmount;

    // 累计部门签单额度 新签部分
    departmentMonthTotalSigningAmountObj[groupId] =
      departmentMonthTotalSigningAmountObj[groupId] ?? 0;
    departmentMonthTotalSigningAmountObj[groupId] += realEffectivePerformanceAmount;
    ...
    return (
      ctx.helper
        ...
        .map((v, i) => {
          ...
          if (!isConfirm) {
            const { realEffectivePerformanceAmount } = v;
            // 该分店对应（设计部门）未确认,才会计算数据
            const {
              effectivePerformanceAmount = 0,
              designFee = 0,
              designPriceAmount = 0,
            } = v.amount;
            ...
            // 根据提点阶段配置计算设计师提点 业绩提点
            const dConfig = tichengRules.designer.performanceTotalRules.find(
              c => monthSigningAmount >= c.min * c.unit && monthSigningAmount <= c.max * c.unit
            );
            const designTichengAmount_tc_ffb = 20;
            const designTichengAmount_zz_yj_td = dConfig?.percent ?? 0;
            const designTichengAmount_pbh_td =
              (designTichengAmount_tc_ffb / 100) * designTichengAmount_zz_yj_td;
            const designTichengAmount_zz_yj_bc_tc =
              (effectivePerformanceAmount * designTichengAmount_pbh_td) / 100;
            v.designTichengAmount = {
              tc_ffb: designTichengAmount_tc_ffb,
              zz_yj_td: designTichengAmount_zz_yj_td,
              pbh_td: designTichengAmount_pbh_td,
              sjs_bc_td: designTichengAmount_pbh_td,
              zz_yj_bc_tc: toFixed(designTichengAmount_zz_yj_bc_tc),
              sjs_tc_hj: toFixed(designTichengAmount_zz_yj_bc_tc),
            };

            // 根据提点阶段配置计算设计部门经理提点
            const dLConfig = tichengRules
              .getDesignLeaderConfig(departmentUsers)
              .performanceTotalRules.find(
                c =>
                  v.departmentMonthTotalSigningAmount >= c.min * c.unit &&
                  v.departmentMonthTotalSigningAmount < c.max * c.unit
              );

            const designLeaderTichengAmount_tc_ffb = 100;
            const designLeaderTichengAmount_zz_yj_td = dLConfig?.percent;
            const designLeaderTichengAmount_pbh_td =
              (designLeaderTichengAmount_tc_ffb / 100) * designLeaderTichengAmount_zz_yj_td;
            const designLeaderTichengAmount_zz_yj_tc =
            ...
            v.designLeaderTichengAmount = {
              ...
              zz_yj_td: designLeaderTichengAmount_zz_yj_td,
              pbh_td: designLeaderTichengAmount_pbh_td,
              zz_yj_tc: toFixed(designLeaderTichengAmount_zz_yj_tc),
            };

            v.designLeaderTichengAmount.sjbm_jl_tchj = toFixed(designLeaderTichengAmount_zz_yj_tc);
          }
          return {
  ...
  function getCellData(tableHeaders, sourceData) {
    const getCell = (row, col, value, header, rowData = {}, rows = []) => {
      ...
      const res = {
        field: header.field,
        v: isPercent ? (+value / 100).toFixed(3) : value.toString(),
      };
      const { getF, ...conf } = header?.config || {};
      if (getF) res.f = getF(row + 1, rowData, [...rows]);
  ...
  function getSheets(res) {
    let tableHeaders = [
      ...
      {
        field_name: "业绩额",
        field: "amount.effectivePerformanceAmount",
        config: {
          t: 2,
          s: "Number_Formatter",
        },
      ...
      },
      {
        ...
        type: "percent",
        config: {
          getF: rowIndex => `=Y${rowIndex}*Z${rowIndex}`,
        },
        width: 92,
      },
      {
        ...
        type: "percent",
        config: {
          getF: rowIndex => `=AA${rowIndex}`,
        },
        width: 126,
      },
      {
        field_name: "设计师整装业绩本次提成",
        field: "designTichengAmount.zz_yj_bc_tc",
        config: {
          t: 2,
          s: "Number_Formatter",
          getF: rowIndex => `=AB${rowIndex}*R${rowIndex}`,
        },
        width: 207,
      },
      {
      ...
      {
        field_name: "设计部经理整装业绩提点",
        field: "designLeaderTichengAmount.zz_yj_td",
        type: "percent",
        width: 201,
      },
      {
        ...
        type: "percent",
        config: {
          getF: rowIndex => `=AF${rowIndex}*AG${rowIndex}`,
        },
        width: 110,
      },
      {
        field_name: "设计部经理整装业绩提成",
        field: "designLeaderTichengAmount.zz_yj_tc",
        config: {
          t: 2,
          s: "Number_Formatter",
          getF: rowIndex => `=AH${rowIndex}*R${rowIndex}`,
        },
        width: 205,
      },
      {
    ...
    const sheetName = `${fitUseStr} 竣工业绩确认 - 设计${isConfirm ? "（已录入）" : ""}`;
    return {
      name: sheetName,
      styles: {
        Header_Formatter: {

 ...
 * @flowNodeId 1
 * @updateTime 2025-07-29 16:21:09
 */
...
try {
  ...
  const supplierId =
    await ctx.service.config.getSupplierConfigIdByUserId(+adminId, companyId);
  const [
    {
      1: dict1,
      2: dict2,
  ...
  records.forEach((record, i) => {
          ...
          if (field.ui_type === 'MultiSelect') {
            ...
            if (text_arr.length) {
              ...
              if (valid) {
                ...
                record.fields[column] = '';
                unValidRow.push({
                  message: `字段: ${field.field_name} ${field_value || '-'
                    } 必须是已存在的数据; `,
                  value: field_value,
                  source: { field, column: field.column, fields: record },
                });
    ...
    if (record.fields['一级分类_ID'] && record.fields['二级分类_ID']) {
      if (
        ...
        unValidRow.push({
          message: `字段: 所选二级分类${record.fields['二级分类'] ?? '-'
      ...
      return await ctx.curl(
        `https://qyapi.weixin.qq.com/cgi-bin/message/send?access_token=${access_token_v2}`,
        {
          ...
          data: {
            ...
            textcard: {
              title: '系统通知: 商家发起材料审批',
              description: `<div class="highlight">【品牌】:${recordData.brandName
                }</div><div class="highlight">【材料类型】:${recordData.fitTypeStr ?? '-'

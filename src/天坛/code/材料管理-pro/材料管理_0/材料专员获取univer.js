 ...
 * @flowNodeId 1
 * @updateTime 2025-09-20 10:16:48
 */
...
try {
  ...
  if (saveRecordInfo?.id && saveRecordInfo?.data) {
    ...
    const { user, role } = await ctx.helper.getRole();
    // if (user.id === 5242) {
    //   return dataRes
    // }
    ...
    if (isCanSet || isInternalSuppliers) {
      list =
        isCanSet && isInternalSuppliers ? [10, 11, 12, 13] : [9, 10, 11, 12];
    } else {
      list = [8, 9, 10, 11];
    }
  ...
  async function getSheetData() {
    let sql = `
    SELECT
        m.*, typeClass.name as typeClassStr, class.name as classStr, brand.name as brandStr,unit.name as unitStr, distribution.name as distributionStr,supplier.name as supplierStr,

/**
 * @flowId 514
 * @flowKey n5z2neewj0r3ejw7
 * @flowName 获取材料专员材料编辑表格univer数据
 * @flowNodeName 函数名称
 * @flowNodeType funEditNode
 * @flowNodeId 1
 * @updateTime 2025-09-20 09:40:52
 */
...
try {
  ...
  const readOnlyFields = [
    'number', // 编码（原本就不可编辑）
    'purchasePrice', // 采购价（新增不可编辑，通过公式计算）
  ];
  ...
  // 生成公式的函数 - 返回getF函数
  function getFormulaForField(fieldKey) {
    const kvs = getKvs();
    const fieldKeys = Object.keys(kvs);

    if (fieldKey === 'purchasePrice') {
      // 采购价 = 未税采购价 * (1 + 税点/100)
      const untaxedPurchasePriceIndex = fieldKeys.indexOf(
        'untaxedPurchasePrice',
      );
      const taxRateIndex = fieldKeys.indexOf('taxRate');

      if (untaxedPurchasePriceIndex !== -1 && taxRateIndex !== -1) {
        const untaxedPurchasePriceCol = getExcelColumn(
          untaxedPurchasePriceIndex,
        );
        const taxRateCol = getExcelColumn(taxRateIndex);
        return (rowIndex) =>
          `=${untaxedPurchasePriceCol}${rowIndex}*(1+${taxRateCol}${rowIndex}/100)`;
      }
    }
  ...
  function getResources(dataResLength) {
    let departmentColIndex = isCanSet ? 10 : 9; // 新增了未税采购价和税点字段，索引+2
    ...
    const getReadOnlyColumns = () => {
      ...
      readOnlyFields.forEach((fieldKey) => {
        const columnIndex = fieldKeys.indexOf(fieldKey);
        if (columnIndex !== -1) {
          readOnlyColumns.push(columnIndex);
        }
      });
  ...
  if (saveRecordInfo?.id && saveRecordInfo?.data) {
    ...
    const maxLength = Object.values(
      sheetData.sheets['sheet-01'].cellData[0],
    )?.length;
    const cellData = Object.values(sheetData.sheets['sheet-01'].cellData)?.map(
      (v) => {
    ...
    let isCanSetIndex = isInternalSuppliers ? 11 : 10; //可使用店面 索引（新增了未税采购价和税点字段，索引+2）
    let isInternalSuppliersIndex = 1; //关联编码 索引
    let isInterFlag = cellData[0][isInternalSuppliersIndex]?.v === '关联编码'; //当前数据是否有关联编码
    let isFlagCan =
      cellData[0][
        !isInternalSuppliers && isInterFlag ? isCanSetIndex + 1 : isCanSetIndex
      ]?.v === '可使用店面'; //当前数据是否有可使用店面
    ...
    cellData.forEach((row, rowIndex) => {
      ...
      Object.keys(kvs).forEach((fieldKey, colIndex) => {
        if (row[colIndex]) {
          // 添加公式支持
          const getF = getFormulaForField(fieldKey);
          if (getF) {
            const formula = getF(rowIndex + 1);
            row[colIndex].f = formula;
            row[colIndex].v = ''; // 公式字段的值由公式计算
          }
        }
      });
    });
    ...
    dataList.forEach((item, index) => {
      ...
      Object.keys(kvs).forEach((v, i) => {
        ...
        cellData[index][i] = {
          ...
          v:
            index === 0
              ? item[i] // 表头行：item是字符串数组，直接取索引i的值
        ...
        if (index !== 0) {
          // 添加公式支持
          const getF = getFormulaForField(v);
          if (getF) {
            const formula = getF(index + 1);
            cellData[index][i].f = formula;
            cellData[index][i].v = ''; // 公式字段的值由公式计算，初始值为空
          }
        }
      });
    });
  ...
  async function getBatchSaleMethodStr(materials) {
    // 收集所有saleMethod值
    const allSaleMethodIds = new Set();
    const materialSaleMethods = {};

    materials.forEach((material) => {
      if (material.saleMethod) {
        const ids = material.saleMethod.split(',').filter((id) => id.trim());
        materialSaleMethods[material.id] = ids;
        ids.forEach((id) => allSaleMethodIds.add(id.trim()));
      }
    });

    if (allSaleMethodIds.size === 0) {
      return {};
    }

    // 批量查询所有products
    const productSql = `SELECT id, name FROM products WHERE id IN (${Array.from(
      allSaleMethodIds,
    )
      .map(() => '?')
      .join(',')})`;
    const products = await ctx.model.query(productSql, {
      replacements: Array.from(allSaleMethodIds),
    ...
    // 为每个material构建saleMethodStr
    const result = {};
    Object.keys(materialSaleMethods).forEach((materialId) => {
      const saleMethodNames = materialSaleMethods[materialId]
        .map((id) => productMap[id])
        .filter((name) => name);
      result[materialId] = saleMethodNames.join(',');
    });
  ...
  async function getSheetData() {
    ...
    sql += sqlText;
    if (brand === '351' && type === 1 && isMainMaterial) {
      sql += " AND (m.status = 1 OR m.createdAt > '2024-12-31 00:00:00')";
    }
    sql += ' GROUP BY m.id';
    ...
    dataRes.forEach((item) => {
      item.saleMethodStr = item.saleMethodStr?.join(',');
    });
    ...
    try {
      ...
      const saleMethodMap = await getBatchSaleMethodStr(dataRes);
      dataRes.forEach((item) => {
        item.saleMethodStr = saleMethodMap?.[item.id] || '';
      });
    } catch (error) {
      console.log('获取数据错误', error);
      throw new Error('获取数据错误', error);
    }
    const { data } = await ctx.model.Record.findOne({
      where: { id: targetId },
    });

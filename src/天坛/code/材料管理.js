 ...
 * @flowNodeId 1
 * @updateTime 2025-09-16 18:10:36
 */
...
try {
  ...
  const readOnlyFields = [
    'number', // 编码（原本就不可编辑）
    'untaxedPurchasePrice', // 未税采购价（新增不可编辑，通过公式计算）
  ];
  ...
  function getFormulaForField(fieldKey) {
    ...
    if (fieldKey === 'untaxedPurchasePrice') {
      // 未税采购价 = 采购价 * (1 - 税点/100)
      const purchasePriceIndex = fieldKeys.indexOf('purchasePrice');
      const taxRateIndex = fieldKeys.indexOf('taxRate');

      if (purchasePriceIndex !== -1 && taxRateIndex !== -1) {
        const purchasePriceCol = getExcelColumn(purchasePriceIndex);
        const taxRateCol = getExcelColumn(taxRateIndex);
        return (rowIndex) =>
          `=${purchasePriceCol}${rowIndex}*(1-${taxRateCol}${rowIndex}/100)`;
      }
    }
  ...
  if (saveRecordInfo?.id && saveRecordInfo?.data) {
    ...
    if (isCanSet || isInternalSuppliers) {
      list =
        isCanSet && isInternalSuppliers ? [12, 13, 14, 15] : [11, 12, 13, 14];
    } else {
      list = [10, 11, 12, 13];
    }
